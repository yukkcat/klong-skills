...
- 定期检查 GitHub 版本标签，发现新版时提醒并生成安全的 Skill 更新指令
...
把本仓库导入 Vercel 后直接部署即可，根目录的 `vercel.json` 已包含构建命令和输出目录，不需要在 Vercel 配置 API Key 或其他环境变量。同一套 Vue 页面会自动判断运行环境：由 Python 启动时使用本地 API、共享图库和 Codex 历史；静态部署时使用浏览器运行时并直接请求小恐龙 API。浏览器版点击“同步”也会直接从 `yukkcat/image-prompts` 拉取并校验最新 registry；随构建发布的快照用于首次显示和网络失败时回退。
